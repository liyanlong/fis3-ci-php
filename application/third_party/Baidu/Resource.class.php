...
function scriptEnd() {
    ...
    if(preg_match($reg,$script,$matches)){
        ...
        Baidu_FIS_Resource::addScriptPool($script);
    }
}
...
function styleEnd() {
    ...
    if(preg_match($reg,$style,$matches)){
        Baidu_FIS_Resource::addStylePool($matches[2]);
    }else{
        Baidu_FIS_Resource::addStylePool($style);
    }
}
...
function framework($id){
    Baidu_FIS_Resource::setFramework(Baidu_FIS_Resource::getUri($id));
}
...
function placeholder($type) {
    echo Baidu_FIS_Resource::placeholder($type);
}
...
function widget($id, $args = array()) {
    $uri = Baidu_FIS_Resource::getUri($id);
    if (is_file($uri)) {
        extract($args);
        include $uri;
        Baidu_FIS_Resource::load($id);
 ...
 * @return [type]        [description]
 */
function display($id, $array = array()) {
    $path = Baidu_FIS_Resource::getUri($id);
    if (is_file($path)) {
        extract($array);
        ob_start();
        include $path;
        $html = ob_get_clean();
        Baidu_FIS_Resource::load($id); //注意模板资源也要分析依赖，否则可能加载不全
        echo Baidu_FIS_Resource::renderResponse($html);
    } else {
